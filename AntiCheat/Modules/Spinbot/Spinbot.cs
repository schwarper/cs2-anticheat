...
namespace AntiCheat.Modules.Spinbot;

public class Spinbot : ICheatDetector
{
    ...
    {
        PlayerData.Get(attacker).Spinbot.RecentlyKilled = true;
    }

    public void OnProcessUsercmds(CCSPlayerController player, QAngle angle)
    {
        SpinbotData data = PlayerData.Get(player).Spinbot;
